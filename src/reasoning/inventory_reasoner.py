...
class InventoryReasoner(BaseReasoner):
    ...
    def generate_actions(self):
        if self.business_state is None:
            raise RuntimeError("Business state is missing. Call analyze() first.")

        inventory = self.business_state.metrics.get("inventory", 100)
        demand = self.business_state.market.get(
            "adjusted_demand",
            self.business_state.market.get("demand", 50),
        )
        product_id = self.business_state.metrics.get("product_id", "UNKNOWN_PRODUCT")
        ...
        if inventory < 20:
            self.action = BusinessAction(
                ...
                value=0,
                unit="UNITS",
                rationale="Inventory is critically low; stock should be protected from rapid depletion",
                confidence=0.90,
                risk=0.10,
                priority=9,
                metadata={
            ...
            self.action = BusinessAction(
                ...
                value=20,
                unit="PERCENT",
                rationale="Inventory is high while demand is low; stock clearance is recommended",
                confidence=0.88,
                risk=0.15,
                priority=8,
                metadata={
                    "recommended_price_operation": "DECREASE",
                    "recommended_price_value": 10,
                },
            )
        else:
            # No price metadata here. MAINTAIN_STOCK is an inventory action,
            # not a zero-percent price recommendation.
            self.action = BusinessAction(
                agent_id="InventoryAgent",
                action_type="INVENTORY_POLICY",
                ...
                value=0,
                unit="UNITS",
                rationale="Inventory and demand are within acceptable levels",
                confidence=0.70,
                risk=0.05,
                priority=5,
                metadata={},
